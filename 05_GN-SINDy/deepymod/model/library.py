...
class Library1D(Library):
    ...
    def library(
        ...
        if len(poly_list) == 1:
        ...
        else: # we are here for the coupled ODE system
            theta_uv = reduce(
                (lambda x, y: (x[:, :, None] @ y[:, None, :]).view(samples, -1)),
                poly_list,
            )   # calculate all unique combinations of polynomials
            ...
            #     ],
            #     1,
            # )  # calculate all unique products of polynomials and derivatives.
            # # This term was absent in DeePyMoD original repo but it is necessary for identification of Keller Segel
